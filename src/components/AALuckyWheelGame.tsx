...
import EoaSignTransaction from "./EoaSignTransaction";
import EoaSignMessage from "./EoaSignMessage";
import EoaSignAndSendTransaction from "./EoaSignAndSendTransaction";
...
const LuckyWheelGame = () => {
  ...
  const spinWheel = async (chainId: number) => {
    ...
    try {
      ...
      const response = await fetch(`${BUNDLER_ENDPOINT}smart-account/send`, {
        method: "POST",
        headers: {
  ...
  const spinWheelAoe = async () => {
    const chainId = 2484;
    const nonce = Math.floor(Math.random() * 1000000);
    if (spinning || !isAuthenticated || !user) return;
    setLoading(true);
    setTransactionState({
      isSuccess: false,
      status: "Initiating smart contract transaction...",
    ...
    if (!response.ok) {
      alert("Failed to sign and send transaction");
    }
    const txData = await response.json();
    setTransactionState({
      isSuccess: false,
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center">
                <span className="text-blue-700 font-medium text-sm">{user?.name?.charAt(0).toUpperCase() || "U"}</span>
              </div>
            ...
            >
              {segments.map((segment, index) => {
                ...
                return (
                  ...
                  >
                    <span className="relative text-xs h-full" style={{top: "30px"}}>
                      {segment.text}
                    </span>
                  </div>
                );
          ...
          {user && (
            <div>
              ...
              <p>eoa</p>
              <EoaSignTransaction user={user} />
              <EoaSignMessage />
              <EoaSignAndSendTransaction onSpin={spinWheelAoe} />
            </div>
          )}
