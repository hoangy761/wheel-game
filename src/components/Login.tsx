...
const Login = ({onLoginStart}: LoginProps) => {
  ...
  const handleOAuthLogin = async (provider: string) => {
    ...
    try {
      ...
      await axios.get(`${BUNDLER_ENDPOINT}auth/init-client`, {
        headers: {
          "x-api-key": API_KEY,
        },
      ...
      const response = await axios.get(`${BUNDLER_ENDPOINT}auth/${provider}`, {
        method: "GET",
        headers: {
          "x-api-key": API_KEY,
        },
  ...
  return (
    <div className="w-full max-w-md mx-auto p-6 bg-white rounded-xl shadow-md">
      ...
      <div className="mt-8 pt-6 border-t border-gray-200 text-center text-xs text-gray-500">
        ...
        <p className="mt-2">
          All transactions secured on-chain with Universal Account.
        </p>
      </div>
    </div>
  );
