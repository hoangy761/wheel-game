...
const abi = ["event SpinCompleted(address indexed player, uint256 indexed spinIndex, uint256 segment, uint256 prize)"];
...
export function decodeSpinCompletedEvent(receipt: any, contractAddress: string) {
  ...
  try {
    ...
    const manualLog = receipt.logs.find(
      (log: any) => log.address.toLowerCase() === contractAddress && log.topics[0] === spinCompletedTopic
    );
    ...
    if (manualLog) {
      ...
      return {
        ...
        formattedPrize: ethers.utils.formatUnits(ethers.BigNumber.from("0x" + manualLog.data.slice(66)), 18),
      };
    }
